...
@section('nav')
<li>
    <a class="block px-4 py-2.5 text-slate-800 font-semibold  hover:bg-emerald-950 hover:text-white rounded-lg" href="{{ url('/home') }}">Home</a>
</li>
<li>
...
<li class="bg-slate-50 pb-2 rounded-lg border">
  ...
  <ol class="text-sm text-slate-700 space-y-4 pl-6 my-2.5">
      ...
      <li>
          <a class="block text-slate-500 hover:text-slate-950">Affiliation</a>
      </li>
  </ol>
</li>
@endsection
...
@section('3')
<li class="before:mx-2">Edit Generation</li>
@endsection
...
<form action="{{ route('generation.update',$generation->id) }}" method="POST" enctype="multipart/form-data">
@csrf
@method('PUT')
...
<div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-3 ">
    <div class="form-group">
        ...
        <input type="text" name="generation_name" placeholder="generation_name" value="{{ $generation->generation_name}}"
    ...
    <div class="form-group">
        ...
        <input type="file" name="generation_img" placeholder="generation_img" value="{{ $generation->generation_img }}"
        class="form-control block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
        @error('generation_img')
        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
        @enderror
    </div>
