...
<a href="{{ route('members.create') }}"><button type="button" class="ml-6 mt-6 text-white bg-cyan-500 hover:bg-cyan-700 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-cyan-500 dark:hover:bg-cyan-600 focus:outline-none dark:focus:ring-cyan-600">ADD TALENTS</button></a>
  ...
  <div class="overflow-hidden rounded-lg border border-gray-200 shadow-md m-5">
      <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
        <thead class="bg-gray-50 ">
          ...
          <th scope="col" class="px-6 py-4 font-medium text-gray-900">S.NO</th>
            ...
            <th scope="col" class="px-6 py-4 font-medium text-gray-900">Affilition</th>
            <th scope="col" class="px-6 py-4 font-medium text-gray-900">Gender</th>
            <th scope="col" class="px-6 py-4 font-medium text-gray-900">YouTube</th>
            ...
            <th class="flex gap-3 px-6 py-4 font-normal text-gray-900">
              <div class="relative h-16 w-16">
                <img
                  ...
                  src={{ asset('images/Portrait/'.$item->members_img_icon) }}
                  alt=""
                />
                
              </div>
              <div class="text-sm">
                <div class="font-medium text-gray-700">{{ $item->members_name }}</div>
                <div class="text-gray-400">{{ $item->generation->generation_name}}</div>
              </div>
            </th>
            <td class="px-6 py-4">
              <div class="font-medium text-gray-400">{{ $item->affiliation->affiliation_name}}</div>
            </td>
            <td class="px-6 py-4 font-medium text-gray-400">{{ $item->gender->gender_name}}</td>
            <td class="px-6 py-4 font-medium text-gray-400">
              <div class="flex gap-2 ">
                  <a href={{ $item->members_youtube}}>{{ $item->members_youtube_name}}</a>
              </div>
            </td>
            <td class="pX-4 py-4">
      ...
      <div class="ml-6 mr-6">{!! $memberss->links() !!}</div>
